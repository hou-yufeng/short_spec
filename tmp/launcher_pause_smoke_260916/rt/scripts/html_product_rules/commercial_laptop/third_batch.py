...
from html_source_rules import additional_third_batch_values, positive_values, remove_parentheses, values_for

PRODUCT_LINE = "commercial_laptop"
...
def generate(features: dict[str, list[str]]) -> list[tuple[str, str, str]]:
    ...
    graphics_source = "Discrete Graphics Support" if ts else "Graphics"
    add("Graphics", "\n".join(_unique(values_for(features, graphics_source))))
    ...
    for source,name in (("Case Color","Color"),("Case Material","Case Material"),("Material","Material"),("Dimensions (WxDxH)","Dimensions (WxDxH)"),("Weight","Weight")):
        ...
        add(name,"\n".join(_unique(values_for(features,source))))
    if not ts:
    ...
    if PRODUCT_LINE in {"desktop","thinkstation"}:
