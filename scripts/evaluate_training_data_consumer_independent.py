...
DATASET_DIR = REPO_ROOT / "data" / "training_data_consumer"
OUTPUT_DIR = REPO_ROOT / "analysis_output" / "consumer_independent_eval"
GENERATED_TEXT_DIR = OUTPUT_DIR / "generated_shortspec_batch_rule_based_consumer"
SPEC_TEXT_DIR = OUTPUT_DIR / "runtime_spec_text_rule_based_consumer"
ACTUAL_TEXT_DIR = OUTPUT_DIR / "actual_shortdesc_text"
ACTUAL_MANIFEST = OUTPUT_DIR / "actual_shortdesc_manifest.json"
WORKBOOK_PATH = OUTPUT_DIR / "generated_consumer_rule_based_summary.xlsx"
REPORT_PATH = OUTPUT_DIR / "training_data_consumer_independent_evaluation.md"
DETAILS_PATH = OUTPUT_DIR / "training_data_consumer_independent_evaluation_details.json"
...
def build_pairs(dataset_dir: Path) -> list[PairRecord]:
    ...
    if missing:
        raise RuntimeError(f"Incomplete pairs in training_data_consumer: {missing[:10]}")
    return pairs
...
def run_batch_generation() -> subprocess.CompletedProcess[str]:
    command = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "batch_generate_shortspec_excel_rule_based_consumer.py"),
        "--spec-dir",
        str(DATASET_DIR),
        "--glob",
        "*_Spec.PDF",
...
def main() -> None:
    ...
    lines: list[str] = []
    lines.append("# training_data_consumer 全量评估报告")
    lines.append("")
    lines.append("## 评估范围")
    lines.append("")
    ...
    lines.append("- 评估方式：对 `training_data_consumer` 下全部 `*_Spec.PDF` 执行现有规则版批处理程序，再与对应 `*_ShortDesc_AutoLayout.pdf` 的实际文本进行对比。")
