...
DISPLAY_LAUNCHERS = [
    {
        "release_bat": "display_short_spec_generator_commercial_laptops.bat",
        "title": "Lenovo Commercial Laptop Display ShortSpec",
        "script": "batch_generate_display_shortspec_excel_rule_based.py",
        "runtime_text": "runtime_spec_text_display_rule_based_commercial",
        "generated_text": "generated_display_shortspec_rule_based_commercial",
        "output": "display_shortspecs_rule_based_commercial_summary.xlsx",
    },
    {
        "release_bat": "display_short_spec_generator_consumer_laptops.bat",
        "title": "Lenovo Consumer Laptop Display ShortSpec",
        "script": "batch_generate_display_shortspec_excel_rule_based_consumer.py",
        "runtime_text": "runtime_spec_text_display_rule_based_consumer",
        "generated_text": "generated_display_shortspec_rule_based_consumer",
        "output": "display_shortspecs_rule_based_consumer_summary.xlsx",
    },
    {
        "release_bat": "display_short_spec_generator_tablet.bat",
        "title": "Lenovo Tablet Display ShortSpec",
        "script": "batch_generate_display_shortspec_excel_rule_based_tablet.py",
        "runtime_text": "runtime_spec_text_display_rule_based_tablet",
        "generated_text": "generated_display_shortspec_rule_based_tablet",
        "output": "display_shortspecs_rule_based_tablet_summary.xlsx",
    },
    {
        "release_bat": "display_short_spec_generator_desktop.bat",
        "title": "Lenovo Desktop Display ShortSpec",
        "script": "batch_generate_display_shortspec_excel_rule_based_dt.py",
        "runtime_text": "runtime_spec_text_display_rule_based_dt",
        "generated_text": "generated_display_shortspec_rule_based_dt",
        "output": "display_shortspecs_rule_based_dt_summary.xlsx",
    },
]
...
REM   - output one Display-only summary worksheet for all products
...
if exist "%DISPLAY_VENDOR_DEPS%\\pypdf" (
  if defined PYTHONPATH (
    set "PYTHONPATH=%DISPLAY_VENDOR_DEPS%;%PYTHONPATH%"
  ) else (
    set "PYTHONPATH=%DISPLAY_VENDOR_DEPS%"
  )
)
...
def release_dir_for_today(repo_root: Path) -> Path:
    return repo_root / "release" / f"display_short_spec_generator_{datetime.now():%y%m%d}"
...
def find_runtime_source(repo_root: Path, target_runtime: Path) -> Path | None:
    ...
    if release_root.exists():
        for pattern in ("display_short_spec_generator_*", "short_spec_generator_*"):
...
def sync_display_pdf_deps(repo_root: Path, target_runtime: Path) -> None:
    target_vendor = target_runtime / "scripts" / "_display_pdf_deps"
...
def sync_runtime(repo_root: Path, target_runtime: Path) -> None:
    ...
    sync_display_pdf_deps(repo_root, target_runtime)

    (target_runtime / "README_clean_portable.txt").write_text(
        "\n".join(
            [
                "Display ShortSpec folder portable runtime.",
                "",
                "Use the Display .bat files in the parent release folder.",
                "Keep this folder next to those .bat files.",
                "Do not edit generated .bat files by hand; rebuild them from scripts/build_display_shortspec_portable_launchers.py.",
                "",
            ]
        ),
        encoding="utf-8",
...
def write_release_readme(target_dir: Path) -> None:
    (target_dir / "README_display_portable.txt").write_text(
        "\n".join(
            [
                "Display ShortSpec Generator portable release.",
                "",
                "Copy this whole folder to another PC, then put full spec PDFs next to the required .bat launcher.",
                "Run the matching Display .bat file. The output workbook is written next to the launcher.",
                "",
                "Delivered launchers:",
                "- display_short_spec_generator_commercial_laptops.bat",
                "- display_short_spec_generator_consumer_laptops.bat",
                "- display_short_spec_generator_tablet.bat",
                "- display_short_spec_generator_desktop.bat",
                "",
...
def main() -> None:
    ...
    (target_dir / ".display_shortspec_portable_package").write_text(
        "folder_portable_display_only\n",
        encoding="utf-8",
    )

    for config in DISPLAY_LAUNCHERS:
