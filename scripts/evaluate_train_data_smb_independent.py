...
OUTPUT_DIR = REPO_ROOT / "analysis_output" / "smb_independent_eval"
GENERATED_TEXT_DIR = OUTPUT_DIR / "generated_shortspec_batch_rule_based_smb"
SPEC_TEXT_DIR = OUTPUT_DIR / "runtime_spec_text_rule_based_smb"
...
REPORT_PATH = OUTPUT_DIR / "train_data_SMB_independent_evaluation.md"
DETAILS_PATH = OUTPUT_DIR / "train_data_SMB_independent_evaluation_details.json"
...
def run_batch_generation() -> subprocess.CompletedProcess[str]:
    command = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "batch_generate_shortspec_excel_rule_based_smb.py"),
        "--spec-dir",
        str(DATASET_DIR),
        "--glob",
        "*_Spec.PDF",
...
def main() -> None:
    ...
    lines.append("- 评估方式：对 `train_data_SMB` 下全部 `*_Spec.PDF` 执行 SMB 独立规则版批处理程序，再与对应 `*_ShortDesc_AutoLayout.pdf` 的实际文本进行对比。")
